...
public class BusinessGauges {
    ...
    @PostConstruct
    void registrar() {
        gauge("vigencia.contracts.active", "Contratos ativos", contratosAtivos, null);
        gauge("vigencia.contracts.expiring_30d",
        ...
        gauge("vigencia.billings.overdue.amount", "Saldo total em atraso",
                valorEmAtrasoCentavos, "BRL", centavos -> centavos.doubleValue() / 100);
        gauge("vigencia.outbox.pending",
     ...
     */
    @Scheduled(fixedDelayString = "${vigencia.jobs.metrics-refresh-delay:60000}")
